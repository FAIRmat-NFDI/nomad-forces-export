...
def test_search_filters_out_excluded_datasets():
    ...
    q = NomadQuery(elements=["Si", "O"], exclude_datasets=["Alexandria"])
    entry_ids = list(q.search(client=mock_client, max_entries=3))
...
def test_search_respects_max_entries_mid_page():
    ...
    q = NomadQuery(elements=["Si"])
    entry_ids = list(q.search(client=mock_client, max_entries=2))
...
def test_fetch_archives_batches_entry_ids_and_yields_archives():
    ...
    q = NomadQuery(elements=["Si"])
    archives = list(
        q.fetch_archives(client=mock_client, entry_ids=["id1", "id2"], batch_size=50)
    )
...
def test_fetch_archives_splits_into_multiple_batches():
    ...
    list(q.fetch_archives(client=mock_client, entry_ids=entry_ids, batch_size=2))
